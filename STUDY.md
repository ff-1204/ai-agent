...
>
> **실측 (2026-09-03)** — 이 PC에 올라마를 깔고 **`qwen3.5:2b`(2.3B)** 로 다시 확인했습니다. 도구 호출 · 도구 선택 · 병렬 호출이 전부 됩니다(CPU 9.2 tok/s). 설정·수치·모델 고른 근거는 [ch04 SETUP.md](ch04_dev-env/SETUP.md), 검사는 `ch04_dev-env/ollama/check_ollama.py`.
>
...
>
> **패키지는 1~11장 전부 깔려 있습니다** (2026-09-04 · import 검사 **38/38 통과**). 확인 명령:
>
> ```powershell
> python scripts/check-env.py
> ```
>
> **따라서 아래 표의 `✅`/`⚠️` 는 이제 순수하게 키 문제입니다.** 패키지 때문에 막히는 장은 없습니다.
>
> 다만 **`supabase` 계열 9개만 pip 로 들어갔습니다.** conda-forge 의 `postgrest` 레시피가 `httpx<0.28` 을 요구해 `a2a-sdk`(`httpx>=0.28.1`)와 충돌하기 때문입니다 — 업스트림에는 없는 제약입니다. 자세한 사정은 [CLAUDE.md](CLAUDE.md) §5.
>
