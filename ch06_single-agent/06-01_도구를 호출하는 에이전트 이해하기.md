...
> **여기서부터 올라마는 모델을 가려야 합니다.** 4·5장은 `invoke()` 한 번이라 아무 모델이나 됐지만, **`bind_tools`는 모델이 도구 호출을 지원해야** 동작합니다. 지원하지 않으면 `tool_calls`가 **빈 리스트로 오고**, 모델은 그냥 말로 답합니다 — 오류가 안 나서 더 헷갈립니다.
>
> ```powershell
> python scripts/check-ollama.py
> ```
>
> 이 스크립트의 **[2] 도구 호출**과 **[3] 도구 두 개 중 고르기**가 통과해야 이 장을 올라마로 진행할 수 있습니다([4.2절](../ch04_dev-env/SETUP.md)).

## 응답이 두 갈래가 됩니다

도구를 붙이기 전에는 [4.4절](../ch04_dev-env/SETUP.md)처럼 `.content`에 답이 왔습니다. 도구를 붙이면 **응답이 둘 중 하나**가 됩니다.
...
[4.4절](../ch04_dev-env/SETUP.md)에서 "`.tool_calls`는 6장에서 채워집니다"라고 한 그 자리입니다.
