...
### 올라마로 바꾸려면 — 임베딩 모델을 따로 받습니다

**이 절은 Chroma가 로컬이라 API 키 없이 됩니다.** 다만 임베딩은 **대화 모델로 안 됩니다.** 임베딩 전용 모델이 따로 있어야 합니다.
...
vectorstore = Chroma(
    persist_directory=DB_PATH,
    embedding_function=OllamaEmbeddings(model=os.getenv("OLLAMA_EMBED_MODEL")),
    collection_name="korean_pdf",
)
```

> **한국어 문서라면 다국어 모델이어야 합니다.** 같은 한국어 문장 4개를 넣고 검색시킨 결과입니다.
>
> **실측 (2026-08-31 · Chroma + 올라마)**
>
> | 모델 | 차원 | 한국어 | 영어 |
> | :--- | ---: | :--- | :--- |
> | `nomic-embed-text` | 768 | **0/3** | 3/3 |
> | `bge-m3` | 1024 | **3/3** | — |
>
> `nomic-embed-text`는 *"도구를 외부에서 쓰려면?"* 에 **"오늘 점심은 김치찌개"** 를 꺼냈습니다. 영어로 바꾸면 정확히 찾습니다 — **모델이 나쁜 게 아니라 언어가 안 맞는 것**입니다.

**여기서 이 절의 함정 하나가 실제로 드러납니다.**

> **임베딩 모델을 바꾸면 벡터 저장소를 다시 만들어야 합니다.** 차원이 달라지기 때문입니다 — OpenAI는 1536, `bge-m3`는 1024. `chroma_db` 폴더를 지우고 인덱싱을 다시 돌리세요.
>
> **검색 결과가 이상하면 모델부터 의심하세요.** 차원이 안 맞으면 오류가 나지만, **언어가 안 맞으면 오류 없이 엉뚱한 것을 가져옵니다.** 이쪽이 훨씬 찾기 어렵습니다.

> **`DB_PATH`가 `./rag_agent/…` 로 시작합니다.** [4.1절](../ch04_dev-env/SETUP.md)의 작업 디렉터리 문제입니다. **`examples/`에서 실행하는 것을 전제로 한 경로**라, `rag_agent/` 안에서 직접 돌리면 DB를 못 찾습니다.
