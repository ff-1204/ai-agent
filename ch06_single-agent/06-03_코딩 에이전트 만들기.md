...
**함수 이름 → `name`, docstring → `description`, 타입 힌트 → `type`, 기본값 없는 인자 → `required`.** 데코레이터가 하는 일이 이게 전부입니다.

> **docstring을 안 쓰면 `description`이 빕니다.** 모델은 이 설명만 보고 도구를 고릅니다. **설명이 부실하면 안 부르거나 엉뚱할 때 부릅니다.** 코드 주석이 아니라 **모델에게 주는 사용 설명서**라고 생각하세요.
...
**설명이 정말 모델에게 전달되는지** 확인했습니다.

```python
@tool
def search_docs(
    query: Annotated[str, Field(description="찾고 싶은 내용. 한국어로 적습니다.")],
    top_k: Annotated[int, Field(description="가져올 문서 수", ge=1, le=10)] = 3,
) -> str:
    """사내 문서를 검색합니다. 최신 뉴스에는 쓰지 마세요."""
```

> **실측 (2026-08-31 · langchain-core 1.6.1)**

```
description: 사내 문서를 검색합니다. 최신 뉴스에는 쓰지 마세요.
- query: {'description': '찾고 싶은 내용. 한국어로 적습니다.', 'type': 'string'}
- top_k: {'default': 3, 'description': '가져올 문서 수',
...
- **설명이 그대로 실렸습니다.** 한국어도 됩니다
- **`ge=1, le=10`이 `minimum`/`maximum`으로 번역됐습니다.** 범위 제약도 모델에게 전달됩니다
- **기본값이 있는 `top_k`는 `required`에서 빠졌습니다.** 모델이 생략할 수 있다는 뜻입니다

> **docstring에 "최신 뉴스에는 쓰지 마세요" 같은 문장을 넣는 것이 효과적입니다.** 도구를 **언제 쓰지 말아야 하는지**까지 적으면 오호출이 줄어듭니다.

> **`Field`를 기본값 자리에 바로 쓰는 방식**(`code: str = Field(...)`, 저자 예제)**과 `Annotated`로 감싸는 방식**(위 실측)**이 둘 다 통용됩니다.** 최신 문서는 `Annotated`를 권합니다 — 기본값 자리를 실제 기본값에 쓸 수 있기 때문입니다.
...
같은 구조의 작은 도구로 확인했습니다.

> **실측 (2026-08-31 · langchain-core 1.6.1)**

```
정상 : 1024
실패 : 실행 실패: SyntaxError: invalid syntax (<string>, line 1)
```

**실패해도 예외가 아니라 문자열이 돌아옵니다.** 프로그램은 계속 돕니다.

반대로 **인자를 빼먹으면** 도구 안으로 들어가기 전에 걸립니다.

```
multiply.invoke({'a': 6}) -> ValidationError: 1 validation error for multiply
```

> **두 실패는 성격이 다릅니다.** `ValidationError`는 **명세 위반**이라 도구 코드가 실행조차 안 됩니다. 반면 도구 안에서 난 오류는 **내가 잡아서 문자열로 바꿔야** 모델이 고칠 기회를 얻습니다. `try/except`를 빼먹으면 그래프가 거기서 죽습니다.
