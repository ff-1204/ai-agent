...
> **이 절은 올라마로 그대로 됩니다.** 노드가 `llm.invoke(question)` 한 번만 부르고 **도구를 쓰지 않기** 때문입니다. 아래 첫 줄만 `ChatOllama`로 바꾸면 나머지는 손댈 필요가 없습니다([4.4절](../ch04_dev-env/04-04_LLM%20%EC%82%AC%EC%9A%A9%ED%95%98%EA%B8%B0.md)).
