...
[5.2절](05-02_%EB%9E%AD%EA%B7%B8%EB%9E%98%ED%94%84%20%EA%B8%B0%EB%B3%B8%20%EA%B0%9C%EB%85%90%20%EC%9D%B4%ED%95%B4%ED%95%98%EA%B3%A0%20%EC%A0%81%EC%9A%A9%ED%95%98%EA%B8%B0.md)에서 부품을 하나씩 봤습니다. 이제 **실제로 도는 그래프**를 만듭니다. [4.4절](../ch04_dev-env/SETUP.md)에서 부른 `llm.invoke()`가 노드 안으로 들어갑니다.
...
> **이 절은 올라마로 그대로 됩니다.** 노드가 `llm.invoke(question)` 한 번만 부르고 **도구를 쓰지 않기** 때문입니다. 아래 첫 줄만 `ChatOllama`로 바꾸면 나머지는 손댈 필요가 없습니다([4.4절](../ch04_dev-env/SETUP.md)).
...
[4.4절](../ch04_dev-env/SETUP.md)에서 본 `.content`가 여기서 쓰입니다. `invoke()`가 돌려주는 것은 객체이므로 텍스트를 꺼내 상태에 넣습니다.
