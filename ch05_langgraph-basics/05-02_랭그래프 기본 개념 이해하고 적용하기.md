...
s: S1 = {"count": "숫자가 아님", "없는키": 123}
print(s)
```

```
선언은 count:int, name:str 인데
  실제로 넣은 값: {'count': '숫자가 아님', '없는키': 123}
  타입 오류가 났나: 아니오. 실행이 여기까지 왔습니다.
```

**타입이 틀린 것도, 선언에 없는 키(`없는키`)도, 빠진 키(`name`)도 전부 통과했습니다.** `TypedDict`는 **주석**일 뿐입니다.

> 그래서 상태에 이상한 값이 들어가도 **그 자리에서 안 터지고 한참 뒤 엉뚱한 노드에서 터집니다.** 랭그래프 디버깅이 어려운 이유 중 하나입니다.
...
**같은 그래프를 리듀서만 바꿔서 돌려 봤습니다.** 노드 둘이 각각 `{"v": 1}`을 반환하고, 시작값은 `10`입니다.

> **실측 (2026-08-31 · langgraph 1.2.11)**

```
리듀서 없음                 시작 10 -> 노드 둘이 각각 1 반환 -> {'v': 1}
Annotated[int, add]    시작 10 -> 노드 둘이 각각 1 반환 -> {'v': 12}
```

**리듀서가 없으면 `1`입니다.** 시작값 10도, 첫 노드의 결과도 전부 사라지고 **마지막에 쓴 값만** 남았습니다. 리듀서를 붙이니 `10 + 1 + 1 = 12`입니다.

> **숫자에도 리듀서를 붙일 수 있습니다.** 리스트 전용이 아닙니다. `Annotated[int, add]`처럼 `operator.add`를 그대로 씁니다.
...
그래프 안에서 실제로 확인했습니다. 노드가 **항상 `id="fixed-1"`인 메시지**를 반환하게 두고 두 번 돌렸습니다.

> **실측 (2026-08-31 · langgraph 1.2.11)**

```
1회 실행 후: [('HumanMessage', 'h-1', '질문'), ('AIMessage', 'fixed-1', '답')]
같은 id 로 또 넣으면: [('HumanMessage', 'h-1', '질문'), ('AIMessage', 'fixed-1', '답')]
```

**두 번 돌렸는데 메시지가 2개 그대로입니다.** 이어 붙였다면 4개가 됐어야 합니다. `id`가 같아서 **교체**된 것입니다.

> **이 `id` 규칙이 나중에 중요해집니다.** 나중에 오래된 메시지를 지우거나 요약본으로 바꿀 때, 같은 `id`로 새 메시지를 보내 **교체**하는 방식을 씁니다. 단순한 이어 붙이기였다면 못 합니다.
>
> **반대로 함정이기도 합니다.** 노드가 `id`를 고정해서 반환하면 대화가 안 쌓입니다. 보통은 `id`를 지정하지 않고 랭체인이 자동 생성하게 둡니다.
...
**에이전트의 루프도 결국 조건부 엣지입니다** — "도구 호출이 있으면 도구 노드로, 없으면 END로". 그 되돌아가는 화살표가 [5.1절](05-01_%EC%99%9C%20%EB%9E%AD%EA%B7%B8%EB%9E%98%ED%94%84%EC%9D%B8%EA%B0%80.md)에서 말한 **순환**입니다.
...
* **`add_messages`는 `id`가 같으면 덮어씁니다.** 메시지를 교체·삭제할 때 이 규칙을 씁니다.
...
* 에이전트의 루프도 결국 **조건부 엣지로 만든 순환**입니다.
...
이제 이걸 **실제로 도는 그래프**로 조립하고, [4.4절](../ch04_dev-env/SETUP.md)에서 부른 `llm.invoke()`를 **노드 안에** 넣어 봅시다. → **[5.3절](05-03_%EB%9E%AD%EA%B7%B8%EB%9E%98%ED%94%84%EB%A1%9C%20%EC%97%90%EC%9D%B4%EC%A0%84%ED%8A%B8%20%EC%84%A4%EA%B3%84%ED%95%98%EA%B3%A0%20%EA%B5%AC%ED%98%84%ED%95%98%EA%B8%B0.md)**
