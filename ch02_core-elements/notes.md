...
| 공식 문서 | 교재 · 필기 | 비고 |
| :--- | :--- | :--- |
| **에이전트 루프(agent loop)** | ReAct | 공식 문서는 `create_agent`가 만드는 루프를 그냥 **agent**라 부름. **ReAct**는 [논문](https://arxiv.org/abs/2210.03629) 용어이고 그대로 통용됨 |
| **Evaluator-optimizer** | Reflection | 워크플로 패턴 5종 중 하나([1장 필기](../ch01_llm-decision/notes.md)). 검색할 때는 이 이름 |
| **도구 호출(Tool calling)** | 도구 호출 | 같음 |
| **도구 스키마(tool schema)** | 도구 리스트 | 모델에게 가는 **이름·설명·인자 명세** |
| **체크포인터(Checkpointer)** | 단기 메모리 | thread 하나 안 (thread-scoped) |
| **스토어(Store)** | 장기 메모리 | thread를 넘어 (cross-thread) |

**본문은 공식 용어로 밀되 교재 용어를 지우지 않음.** 막혔을 때 찾아볼 곳이 공식 문서이므로 노트의 말과 문서의 말이 같아야 함([CLAUDE.md](../CLAUDE.md) §3).
...
- 대표적인 추론 패턴은 **ReAct**와 **Reflection**(공식: **Evaluator-optimizer**). 에이전트가 문제 해결 과정에서 **어떻게 사고를 전개하고, 행동을 선택하며, 결과를 점검하는지** 확인해야 함

> **공식 문서에는 "추론 능력"이라는 항목이 없음.** 대신 **워크플로 패턴 5종**으로 정리해 두었고, 그중 **Evaluator-optimizer**가 Reflection에 해당함. ReAct 쪽은 패턴이 아니라 **에이전트(agent) 그 자체**로 다룸 — 흐름을 미리 그리지 않는 구조이므로([1.4절](../ch01_llm-decision/01-04_LLM%20%EA%B8%B0%EB%B0%98%20%EC%97%90%EC%9D%B4%EC%A0%84%ED%8A%B8%20%EC%8B%9C%EC%8A%A4%ED%85%9C%2C%20%EC%9D%B4%EB%9F%B0%20%EA%B5%AC%EC%A1%B0%EB%A1%9C%20%EC%84%A4%EA%B3%84%EB%90%9C%EB%8B%A4.md)).
...
### Reflection — 공식 문서의 Evaluator-optimizer
...
공식 용어로 **도구 호출(Tool calling)**.

- 에이전트가 더 다양한 작업을 수행하려면 **LLM이 본래 가지지 못한 능력을 외부 도구를 통해 보완**할 필요가 있음
- LLM은 **자신이 사용 가능한 도구 스키마(tool schema)를 참고**하여 사용자 요청에 따라 도구를 호출할 수 있음
- 도구 호출은 LLM에게 **특정 능력을 부여**하여 **기존의 지식으로 해결할 수 없는 문제**를 풀게 하기 위한 기법

### 모델에게 가는 것은 함수가 아님

**이름 · 설명(description) · 인자 스키마**만 감. **함수 본문은 가지 않음**([2.2절](02-02_%EC%97%90%EC%9D%B4%EC%A0%84%ED%8A%B8%EC%9D%98%20%EC%99%B8%EB%B6%80%20%EC%A7%80%EC%8B%9D%20%ED%99%9C%EC%9A%A9%20-%20%EB%8F%84%EA%B5%AC%20%ED%98%B8%EC%B6%9C.md)). 그래서 **독스트링(docstring)이 모델이 고르는 유일한 근거**가 됨 → [1.3절](../ch01_llm-decision/01-03_LLM%EC%9D%B4%20%EB%8B%A4%EC%9D%8C%20%ED%96%89%EB%8F%99%EC%9D%84%20%EA%B2%B0%EC%A0%95%ED%95%98%EB%8B%A4.md)에서 본 것과 같은 이야기.

모델이 도구를 부르겠다고 하면 응답의 **`tool_calls`** 에 담겨 오고, 실행 결과는 **`ToolMessage`** 로 대화에 다시 들어감. 둘 다 공식 용어.
...
| 공식 용어 | 교재 | 범위 | 내용 |
| :--- | :--- | :--- | :--- |
| **체크포인터(Checkpointer)** | 단기 메모리 | **현재 대화 안** (thread-scoped) | 발생한 정보. 이전 답변에 대해 피드백을 주고받으며 **연속적인 대화를 이어갈 수 있음** |
| **스토어(Store)** | 장기 메모리 | **처음 접속 이후 전체** (cross-thread) | 나눈 대화 전체를 기반으로 습득한 정보. 사용자의 **과거 경험과 성향에 기반**하여 답할 수 있음 |

> **8장 전체가 이 두 단어 위에 있음**([2.3절](02-03_%EC%97%90%EC%9D%B4%EC%A0%84%ED%8A%B8%EC%9D%98%20%EA%B8%B0%EC%96%B5%EB%A0%A5%20-%20%EB%A9%94%EB%AA%A8%EB%A6%AC.md)). 문서를 찾을 때는 **단기/장기가 아니라 checkpointer·store**로 검색해야 나옴.

## ReAct 기반 LLM 에이전트 — 공식 문서의 에이전트 루프

- **외부 도구를 호출하는 에이전트**를 구성하며, ReAct 기반의 추론 능력을 지닌 에이전트가 **어떻게 구현되는지** 확인
- 공식 문서에서는 이 구조를 따로 부르지 않고 **에이전트(agent)** 그 자체로 다룸. 6장의 **`create_agent`** 가 아래 그림을 통째로 만들어 줌
...
| | 체크포인터 (단기) | 스토어 (장기) |
| :--- | :--- | :--- |
| 공식 표현 | **thread-scoped** | **cross-thread** |
| 구분 열쇠 | **`thread_id`** | **네임스페이스**에 넣는 `user_id` (직접 정함) |
| 범위 | 대화 하나 안 | 대화를 넘어 |
...
>
> **`user_id`는 프레임워크 용어가 아님.** 스토어의 키는 **네임스페이스(namespace) 튜플**이고, 거기에 무엇을 넣을지는 개발자 몫임 — 관례적으로 `("memories", user_id)`처럼 씀.
