...
| **저장소 전체 의존성을 conda 환경에 넣기** | `mcp`·`a2a-sdk`·`supabase` 등은 conda-forge에 없어 pip를 섞어야 합니다. **4장 범위로 충분**하다고 판단했습니다. 6장 이후 갈 때 그 장 것만 추가합니다 |
...
| **6장 이후 패키지를 conda 환경에 추가** | ⬜ | 지금은 4장 범위(`langgraph`·`langchain`·`langchain-openai`·`python-dotenv`·`langchain-ollama`)만. `scripts/check-env.py` 는 그래서 아직 통과하지 못합니다 |
