...
| 할 일 | 상태 | 비고 |
| :--- | :--- | :--- |
| **2~11장을 특성화고 눈높이로** | ⬜ | 1장만 적용됨. 개념 장(2·3장)부터가 자연스러움 |
| **절 사이 이해 흐름 확대** | ⬜ | 1장에만 있음. [CLAUDE.md](CLAUDE.md) §3의 틀을 사용 |
| **실행 환경 구축** | ⬜ | `uv` 설치 → `uv sync --upgrade` → `.env`. 이후 4장부터 실측 보강 가능 |
| **`uv.lock` 갱신** | ⬜ | 현재 `pyproject.toml`보다 오래됨 |
| **`question/` 규칙 정하고 작성** | ⬜ | 노트가 더 쌓인 뒤 |
| **`notes.md` (장 요약)** | 진행 중 | **직접 쓰는 칸.** 1장 작성됨 |

> **2~11장 표현을 손보기 전에 1장을 먼저 읽어 보는 편이 낫습니다.** 걸리는 표현이 있으면 그 피드백을 반영해야 기준이 맞습니다.
