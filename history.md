...
| **올라마 파일을 4장 루트에 두기** | 문서 4개가 루트에 나란히 있으니 **무엇이 본체인지 안 보였습니다.** `ollama/` 로 모으고 루트에는 `README.md`·`SETUP.md` 둘만 남겼습니다 — "4장은 SETUP.md 하나 따라가면 된다"가 구조로도 드러납니다. 폴더 안에서는 `ollama-` 접두사를 뗐습니다 |
| **파일을 옮기고 링크 검사만 하기** | 링크는 멀쩡한데 **코드가 깨져 있었습니다.** `check_ollama.py` 가 한 단계 깊어지면서 `parent.parent` 가 저장소 루트를 못 가리켜 `.env` 를 놓쳤습니다. **상대 경로를 쓰는 코드는 실행해 봐야 잡힙니다** |
| **4장 문서에 검증 과정을 남겨 두기** | 재현 검증·실측 블록·문제 해결 표가 쌓이면서 4장이 **교재 노트가 아니라 올라마 운영 매뉴얼**이 돼 있었습니다. 783줄을 걷어내고 "파일을 실행하면 된다" 수준으로 줄였습니다. **왜 그렇게 정했는지는 이 문서(⑫·⑬)에 이미 있습니다** — 두 곳에 둘 이유가 없습니다 |
| **`OLLAMA_DEBUG` 로 GIN 로그 끄기** | 안 됩니다. `ERROR` 로 올려도 접근 로그는 그대로입니다(2026-09-04 클러스터 확인). `GIN_MODE` 도 하드코딩돼 무시됩니다([ollama#8682](https://github.com/ollama/ollama/issues/8682)). **프로브 주기를 늘리는 것 외에 방법이 없습니다.** 안 되는 것을 적어 두지 않으면 같은 로그를 보고 또 시도하게 됩니다 |
...
| `4de8f28` | docs: 올라마 서버 문서 분리 판단 기록 + 커밋 이력 부록 갱신 |
| `9ffca8e` | docs(ch04): 학습에 필요한 만큼으로 줄이고 프로브·로그 실측 반영 |
| `b1ecca2` | docs: 4장 축약·GIN 로그 판단 기록 + 커밋 이력 부록 갱신 |
| `dea5e8d` | refactor(ch04): 올라마 관련 4개 파일을 ollama/ 로 모으기 |
