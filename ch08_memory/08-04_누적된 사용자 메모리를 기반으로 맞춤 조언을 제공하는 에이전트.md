...
> **여기까지** — 단기 메모리를 붙이고 관리하는 법까지 왔습니다(8.2 · 8.3절).
> **이 절의 질문** — **대화를 넘어서** 기억하게 하려면?
> **다 읽으면** — **스토어**로 장기 메모리를 만들 수 있습니다. 8.1절의 두 층이 코드에서 **나란히** 붙습니다.
...
> **실측 (2026-08-31 · langgraph 1.2.11 · `InMemoryStore`)**

```python
ns = ("memories", "user-1")
store.put(ns, "food", {"fact": "매운 음식을 좋아함"})
store.put(ns, "job",  {"fact": "특성화고 재학 중"})
```

```
get : {'fact': '매운 음식을 좋아함'}
search (전체):
  food   {'fact': '매운 음식을 좋아함'}
  job    {'fact': '특성화고 재학 중'}
...
**`("memories", "user-2")`로 찾으면 빈 리스트입니다.** 네임스페이스에 `user_id`를 넣었기 때문에 사용자끼리 섞이지 않습니다.

> **`thread_id`와 역할이 다릅니다.** `thread_id`는 **대화 하나**를 가르고([8.2절](08-02_%EB%8C%80%ED%99%94%20%EB%A7%A5%EB%9D%BD%EC%9D%84%20%EC%9D%B4%ED%95%B4%ED%95%98%EB%8A%94%20%EC%97%90%EC%9D%B4%EC%A0%84%ED%8A%B8.md)), 네임스페이스의 `user_id`는 **사람 하나**를 가릅니다. 대화가 끝나도 남아야 하는 것이 스토어입니다.
...
> **메모리가 두 층이라는 걸 알았고(8.1) → 체크포인터로 대화를 이었고(8.2) → 자르기·요약으로 관리했고(8.3) → 스토어로 대화를 넘어 기억하게 했습니다(8.4).**

**[2.3절](../ch02_core-elements/02-03_%EC%97%90%EC%9D%B4%EC%A0%84%ED%8A%B8%EC%9D%98%20%EA%B8%B0%EC%96%B5%EB%A0%A5%20-%20%EB%A9%94%EB%AA%A8%EB%A6%AC.md)에서 이름만 잡아 둔 두 단어가 코드가 됐습니다.**

**여기까지가 Part 03입니다.** 그리고 **에이전트를 만드는 데 필요한 것은 다 배웠습니다.**

9장부터는 **밖으로 넓힙니다.**

| 장 | 무엇을 잇나 |
| :--- | :--- |
| **9장 MCP** | 에이전트 ↔ **남이 만든 도구** |
| **10장 A2A** | 에이전트 ↔ **다른 에이전트** |
| **11장** | 전부 합친 실전 프로젝트 |

> [2.2절](../ch02_core-elements/02-02_%EC%97%90%EC%9D%B4%EC%A0%84%ED%8A%B8%EC%9D%98%20%EC%99%B8%EB%B6%80%20%EC%A7%80%EC%8B%9D%20%ED%99%9C%EC%9A%A9%20-%20%EB%8F%84%EA%B5%AC%20%ED%98%B8%EC%B6%9C.md) 마지막에 **"MCP 도구도 모델 입장에서는 똑같다"** 고 적어 뒀습니다. 그 한 줄을 기억하고 9장에 들어가면 훨씬 쉽습니다.
