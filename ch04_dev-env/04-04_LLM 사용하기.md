...
튜플의 **첫 칸이 클래스를 정합니다.** `"system"` → `SystemMessage`, `"human"` → `HumanMessage`. 6장에서 `ToolMessage`가 여기에 추가됩니다.
...
**`tool_calls`가 빈 리스트로 이미 존재합니다.** 도구를 안 붙였을 뿐 자리는 처음부터 있습니다. 6장에서 `.bind_tools()`를 하면 이 칸이 채워집니다.
