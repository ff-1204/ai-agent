...
**그냥 바꾸면 두 군데서 걸립니다.** 실제로 돌려 보고 찾은 것입니다.

> **실측 (2026-08-31 · langchain-ollama 1.1.0 · 9B 모델, 4비트 양자화)**

**① `reasoning=False` — 안 끄면 느립니다**

요즘 모델은 답하기 전에 **생각을 글로 씁니다.** 그 글도 토큰이라 시간이 그대로 듭니다.

```
"대한민국의 수도는? 도시 이름만 답하세요."

기본(생각 켜짐)   54.1초   출력 227토큰
reasoning=False   1.4초   출력   2토큰
```

**같은 답("서울")에 40배 차이**입니다. 에이전트는 한 요청에 LLM을 여러 번 부르니 이 차이가 그대로 곱해집니다.

**② `method="function_calling"` — 구조화 출력이 깨집니다**

```python
llm.with_structured_output(Category)                            # 기본값
# OutputParserException: Invalid json output: 환불
```

**분류는 맞게 했는데 JSON이 아니라 값만 뱉었습니다.** 형식을 강제하는 방식이 안 통한 것입니다.

```python
llm.with_structured_output(Category, method="function_calling")  # 이렇게
# category='환불'  reason='고객이 상품을 마음에 들어하지 않아 환불을 요청하고 있습니다.'
```

**도구 호출 방식으로 돌리면 됩니다.** 이 모델이 도구 호출은 잘하기 때문입니다.

> **`Literal`로 좁히는 것도 같이 하세요.** `category: str`로 두면 모델이 `"AI Limitation Clarification"` 같은 걸 채웁니다. [1.2절](../ch01_llm-decision/01-02_LLM%EC%9D%84%20%EA%B8%B0%EB%B0%98%EC%9C%BC%EB%A1%9C%20%EC%9D%98%EC%82%AC%EA%B2%B0%EC%A0%95%ED%95%98%EB%8B%A4.md)의 **"선택지를 좁힌다"** 가 작은 모델에서 훨씬 절실합니다.
...
> **직접 확인하세요.** `uv run python scripts/check-ollama.py`가 단순 호출·도구 호출·도구 선택·구조화 출력 넷을 재고, 구조화 출력은 **되는 `method`를 찾아서 알려 줍니다.**
