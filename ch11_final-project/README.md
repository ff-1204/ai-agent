...
> **이 장은 실행 결과를 거의 넣지 못했습니다.** 네 에이전트 전부가 외부 서비스를 요구합니다 — OpenAI · Tavily · Supabase · 구글 드라이브 OAuth. `.env`가 비어 있는 동안에는 서버가 뜨지 않습니다.
>
> **키 없이 확인한 것만 실측으로 넣었습니다** — `common/config.py`의 포트 배치, Artifact 헬퍼 세 종류의 실제 동작, `storage_ref`로 오가는 데이터 크기([11.1절](11-01_%EC%98%A4%EC%BC%80%EC%8A%A4%ED%8A%B8%EB%A0%88%EC%9D%B4%ED%84%B0%20%EA%B8%B0%EB%B0%98%20%EB%A9%80%ED%8B%B0%20%EC%97%90%EC%9D%B4%EC%A0%84%ED%8A%B8%20%EC%84%A4%EA%B3%84.md)).
>
> **A2A 배관 자체는 [10.3절](../ch10_a2a/10-03_A2A%EC%9D%98%20%EA%B8%B0%EB%B3%B8%20%EC%82%AC%EC%9A%A9%EB%B2%95%20%EC%9D%B4%ED%95%B4%ED%95%98%EA%B8%B0.md)에서 실제로 돌려 봤습니다.** 서버가 뜨고 카드가 노출되고 메시지가 오가는 과정은 그쪽에 실측으로 남아 있습니다. 11장은 그 위에 LLM과 외부 API를 얹은 것뿐입니다.

> **올라마로 어디까지 되나** — **이 장은 올라마만으로는 어렵습니다.** 네 에이전트가 Tavily · Supabase · 구글 드라이브를 각각 요구합니다. `orchestrator_agent`와 `common/`은 키 없이 읽고 시험할 수 있습니다.
>
