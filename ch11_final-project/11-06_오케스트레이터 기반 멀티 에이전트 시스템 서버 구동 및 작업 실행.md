...
> **이 저장소에서는 `python`이 아니라 `uv run python`입니다.** 시스템 `python`은 Microsoft Store 스텁이라 아무 일도 일어나지 않습니다([4.1절](../ch04_dev-env/04-01_%EB%B9%84%EC%A3%BC%EC%96%BC%20%EC%8A%A4%ED%8A%9C%EB%94%94%EC%98%A4%20%EC%BD%94%EB%93%9C%20%ED%99%98%EA%B2%BD%20%EC%84%A4%EC%A0%95%ED%95%98%EA%B8%B0.md)). 아래 명령은 교재 그대로 두었으니 앞에 `uv run`을 붙여 읽으세요.
...
포트 배치는 `common/config.py` 한 곳에 모여 있습니다. **코드를 안 읽고 확인하는 법:**

```python
from common.config import AGENT_CONFIGS
for key, c in AGENT_CONFIGS.items():
    print(key, c.port, c.name)
```

> **실측 (2026-08-31)** — 외부 서비스 없이 임포트만 하면 됩니다.

```
orchestrator     10010  Orchestrator Agent
web_research     10011  Web Research Agent
internal_rag     10012  Internal RAG Agent
file_management  10013  File Management Agent
```

**오케스트레이터가 10010으로 제일 앞**이고 작업자가 뒤로 늘어섭니다. 에이전트를 하나 더 붙인다면 `10014`가 자연스러운 자리입니다.

윈도우에서 포트가 비었는지 보려면:

```powershell
Get-NetTCPConnection -LocalPort 10010,10011,10012,10013 -ErrorAction SilentlyContinue
```

아무것도 안 나오면 비어 있는 것입니다.
