...
> **`python` 을 그냥 치면 안 됩니다.** 이 저장소는 conda 환경 `agent` 를 쓰고, PATH에 넣지 않았습니다([SETUP.md §2.3](../ch04_dev-env/SETUP.md)). 아래 명령은 교재 그대로 두었으니 `python` 자리에 환경 파이썬을 넣어 읽으세요 — `…\miniforge3\envs\agent\python.exe`. "Miniforge Prompt" 에서 `conda activate agent` 한 뒤라면 그대로 `python` 으로 됩니다.
...
| [4장](../ch04_dev-env/SETUP.md) | `.env`와 자격증명 관리 |
