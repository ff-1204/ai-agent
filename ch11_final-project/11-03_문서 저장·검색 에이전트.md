...
> **여기까지** — 파일 관리 에이전트를 만들었습니다([11.2절](11-02_%ED%8C%8C%EC%9D%BC%20%EA%B4%80%EB%A6%AC%EB%A5%BC%20%EC%9C%84%ED%95%9C%20%EC%97%90%EC%9D%B4%EC%A0%84%ED%8A%B8.md)).
> **이 절의 질문** — 그 파일들을 **검색할 수 있게** 하려면? [6.5절](../ch06_single-agent/06-05_RAG%EB%A5%BC%20%EC%9C%84%ED%95%9C%20%EC%97%90%EC%9D%B4%EC%A0%84%ED%8A%B8%20%EB%A7%8C%EB%93%A4%EA%B8%B0.md)의 RAG를 **실전 규모**로 키우면?
> **다 읽으면** — **벡터 검색과 SQL 검색을 나누는 판단**을 알게 됩니다. 이 장에서 가장 큰 코드입니다.
...
>
> **확장(Extension)** 은 데이터베이스에 기능을 덧붙이는 플러그인입니다. PostgreSQL은 원래 숫자·문자열은 알아도 **벡터**는 모릅니다. `pgvector`를 켜야 벡터를 넣을 컬럼을 만들 수 있고, "비슷한 것 찾기"도 할 수 있습니다.
>
> **임베딩 차원이 맞아야 합니다.** 임베딩은 문장을 숫자 목록으로 바꾼 것이고, **차원**은 그 목록의 길이입니다. 모델마다 길이가 정해져 있어서, **테이블에 적어 둔 길이와 모델이 내놓는 길이가 다르면 저장 자체가 실패합니다.** `text-embedding-3-small`은 1536차원입니다([8.4절](../ch08_memory/08-04_%EB%88%84%EC%A0%81%EB%90%9C%20%EC%82%AC%EC%9A%A9%EC%9E%90%20%EB%A9%94%EB%AA%A8%EB%A6%AC%EB%A5%BC%20%EA%B8%B0%EB%B0%98%EC%9C%BC%EB%A1%9C%20%EB%A7%9E%EC%B6%A4%20%EC%A1%B0%EC%96%B8%EC%9D%84%20%EC%A0%9C%EA%B3%B5%ED%95%98%EB%8A%94%20%EC%97%90%EC%9D%B4%EC%A0%84%ED%8A%B8.md)의 `dims=1536`과 같은 값). 모델을 바꾸면 테이블도 다시 만들어야 합니다.
...
**안에 있는 것**은 다 찾을 수 있게 됐습니다. **밖에 있는 것은요?**

우리 드라이브에 없는 정보 — 오늘 환율, 최신 뉴스 — 는 웹에서 가져와야 합니다. 그리고 이번엔 **코드가 갑자기 짧아집니다.** → **[11.4절](11-04_%EC%9B%B9%20%EA%B2%80%EC%83%89%20%EC%97%90%EC%9D%B4%EC%A0%84%ED%8A%B8.md)**
