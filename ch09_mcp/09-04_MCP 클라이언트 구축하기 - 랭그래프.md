...
> **`./server.py`가 상대 경로**입니다. **클라이언트를 실행한 디렉터리 기준**이므로 `examples/mcp_agent/`에서 `python client.py`를 돌려야 합니다([4.1절](../ch04_dev-env/SETUP.md)의 작업 디렉터리 문제).
>
> `command="python"`도 주의할 점입니다. **PATH의 `python`이 가상환경의 것인지** 확인하세요([4.2절](../ch04_dev-env/SETUP.md)). 서버가 다른 파이썬으로 뜨면 `mcp` 패키지를 못 찾습니다.
...
> **실측 (2026-08-31 · langchain-mcp-adapters 0.3.2 · mcp 1.29.1)**

```
StructuredTool   name='add'
    description='두 정수를 더합니다.'
    args={'a': {'title': 'A', 'type': 'integer'},
          'b': {'title': 'B', 'type': 'integer'}}
StructuredTool   name='save_note'
    description='메모를 저장하고 저장 결과를 알려 줍니다.'
    args={'title': {'title': 'Title', 'type': 'string'},
          'body': {'title': 'Body', 'type': 'string'}}
```

**`StructuredTool`입니다.** [6.3절](../ch06_single-agent/06-03_%EC%BD%94%EB%94%A9%20%EC%97%90%EC%9D%B4%EC%A0%84%ED%8A%B8%20%EB%A7%8C%EB%93%A4%EA%B8%B0.md)에서 `@tool`로 만든 것과 **완전히 같은 클래스**입니다. 그래서 `create_agent`가 구분 없이 받습니다.

### 연결할 때 서버가 자기를 소개합니다

`session.initialize()`의 반환값에 서버 정보가 들어 있습니다.

```
서버 이름   : demo-server
서버 버전   : 1.29.1
프로토콜    : 2025-11-25
제공 기능   : ['prompts', 'resources', 'tools']
```

> **`제공 기능`을 보면 그 서버가 무엇을 주는지 미리 알 수 있습니다.** 도구만 주는 서버도 있고 프롬프트·리소스까지 주는 서버도 있습니다. [9.2절](09-02_MCP%20%EA%B8%B0%EB%8A%A5%EC%9D%B4%20%EB%93%A4%EC%96%B4%EA%B0%84%20%EC%97%90%EC%9D%B4%EC%A0%84%ED%8A%B8%20%EC%84%9C%EB%B9%84%EC%8A%A4.md)에서 "가져다 쓸 때 확인할 것"이라고 한 항목을 코드로 확인하는 방법입니다.
...
> **`arguments=`는 키워드로만 넘길 수 있습니다.** 위치 인자로 주면 실패합니다.
>
> ```
> TypeError: load_mcp_prompt() takes 2 positional arguments but 3 were given
> ```
>
> **실측 (2026-08-31 · langchain-mcp-adapters 0.3.2)** — 실제 시그니처는 `load_mcp_prompt(session, name, *, arguments=None)`입니다. 저자 예제도 `arguments=`로 쓰고 있으니 그대로 따라 쓰면 됩니다.
>
> 결과는 랭체인 메시지 리스트입니다.
>
> ```
> load_mcp_prompt -> [('HumanMessage', '다음 주제를 세 문장으로 요약하세요: MCP')]
> ```
>
> **서버가 만든 문자열이 `HumanMessage`가 되어 돌아옵니다.** 그래서 바로 `agent.invoke({"messages": prompts})`에 넣을 수 있습니다.
