...
> **다 읽으면** — **MCP 서버 하나 = 에이전트 하나**라는 깔끔한 구조를 보게 됩니다.
...
> **API 키가 URL에 들어갑니다.** `?tavilyApiKey=…` 형태라 로그·에러 메시지에 그대로 찍힐 수 있습니다. [4.3절](../ch04_dev-env/SETUP.md)에서 "키를 출력하지 말라"고 한 이유가 여기서도 적용됩니다. `print(client)` 같은 걸 하지 마세요.
