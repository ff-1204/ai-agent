...
> **올라마로 어디까지 되나** — **올라마로 9.1~9.4가 됩니다.** 직접 만드는 MCP 서버는 외부 서비스가 필요 없습니다. **9.5절만 Tavily 키가 필요합니다.**
>
> 전체 지도는 [STUDY.md](../STUDY.md#올라마로-어디까지-되나)에 있습니다.

## 이 장의 이해 흐름

**Part 04, 밖으로 넓히는 장**입니다. 새 개념이라기보다 **[2.2절](../ch02_core-elements/02-02_%EC%97%90%EC%9D%B4%EC%A0%84%ED%8A%B8%EC%9D%98%20%EC%99%B8%EB%B6%80%20%EC%A7%80%EC%8B%9D%20%ED%99%9C%EC%9A%A9%20-%20%EB%8F%84%EA%B5%AC%20%ED%98%B8%EC%B6%9C.md)의 도구를 밖으로 뺀 것**입니다.

```mermaid
graph TD
    Q0["도구를 매번<br/>직접 만들어야 하나?"] --> S1["<b>9.1</b> MCP란"]
    S1 --> Q1["왜 표준까지?<br/>남의 서버는 안전한가?"]
    Q1 --> S2["<b>9.2</b> MCP 생태계"]
    S2 --> Q2["직접 만들면<br/>그 걱정이 없다"]
    Q2 --> S3["<b>9.3</b> 서버 구축"]
    S3 --> Q3["만든 서버에<br/>어떻게 붙지?"]
    Q3 --> S4["<b>9.4</b> 클라이언트"]
    S4 --> Q4["서버가 여러 개면?"]
    Q4 --> S5["<b>9.5</b> 멀티 서버"]
    S5 --> N["10장 · A2A"]
...
| 절 | 이 절의 질문 | 얻는 것 |
| :--- | :--- | :--- |
| **9.1** | 도구를 매번 직접 만들어야 하나? | MCP = **도구를 밖으로 뺀 것** |
| **9.2** | 왜 표준까지? 안전한가? | **N×M → N+M** · 가져다 쓸 때 확인할 것 |
| **9.3** | 직접 만들려면? | `@mcp.tool()` — **6.3절과 거의 같음** |
| **9.4** | 어떻게 붙지? | **두 줄**이면 `create_agent` 에 |
| **9.5** | 서버가 여러 개면? | **서버 하나 = 에이전트 하나** |

> **이 한 줄만 붙잡으면 9장이 쉽습니다** — **모델 입장에서는 직접 만든 도구와 MCP 도구가 똑같습니다.** 어디서 왔든 이름·설명·인자 스키마를 받아 고를 뿐입니다.

### 9장 전체를 한 줄로

> **MCP가 도구를 밖으로 뺀 것임을 알았고(9.1) → 왜 표준인지와 주의점을 봤고(9.2) → 직접 서버를 만들었고(9.3) → 클라이언트로 붙였고(9.4) → 서버 여러 개를 멀티 에이전트로 엮었다(9.5).**
