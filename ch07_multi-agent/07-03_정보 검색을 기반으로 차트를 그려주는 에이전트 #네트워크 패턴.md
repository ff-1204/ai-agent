...
> **여기까지** — `Command` 로 일을 넘길 수 있게 됐습니다([7.2절](07-02_%ED%95%B8%EB%93%9C%EC%98%A4%ED%94%84%EB%A5%BC%20%EC%9C%84%ED%95%9C%20%EA%B8%B0%EB%8A%A5%20%EC%82%B4%ED%8E%B4%EB%B3%B4%EA%B8%B0.md)).
> **이 절의 질문** — **관리자 없이** 둘이 주고받게 하면 어떻게 되지?
> **다 읽으면** — 네트워크 패턴을 만들 수 있고, **이 패턴의 약점**(종료 판단)도 알게 됩니다.
...
`filename`을 모델이 정하고 상대 경로로 저장하므로, **`langgraph dev`를 실행한 디렉터리에 파일이 생깁니다**([4.1절](../ch04_dev-env/SETUP.md)의 작업 디렉터리 문제).
...
돌아가긴 합니다. **그런데 종료 판단이 `"최종 답변" in content` 입니다.**

모델이 "최종 답변을 드리자면…"이라고 자연스럽게 쓰면 의도치 않게 끝나고, 약속어를 잊으면 안 끝납니다. **불안하죠.**

**관리자를 하나 두면** 이 문제가 풀릴까요? → **[7.4절](07-04_%EC%9B%B9%ED%8E%98%EC%9D%B4%EC%A7%80%EB%A5%BC%20%EC%9A%94%EC%95%BD%ED%95%B4%EC%84%9C%20%EB%8D%B0%EC%9D%B4%ED%84%B0%EB%B2%A0%EC%9D%B4%EC%8A%A4%EC%97%90%20%EC%A0%80%EC%9E%A5%ED%95%98%EB%8A%94%20%EC%97%90%EC%9D%B4%EC%A0%84%ED%8A%B8%20%23%EC%8A%88%ED%8D%BC%EB%B0%94%EC%9D%B4%EC%A0%80.md)**
